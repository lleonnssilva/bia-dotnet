...
aws ecr get-login-password --region us-east-1 --profile bia | docker login --username AWS --password-stdin 211125447912.dkr.ecr.us-east-1.amazonaws.com/bia
docker build -t bia .
...
git checkout docker-compose.ymls